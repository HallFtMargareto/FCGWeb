<template>
  <div>
          ...
          <el-col :span="20">
            <div>
              <el-form-item label="省份ID">
                <el-input
                  placeholder="省份ID"
                  v-model="searchInfo.province_id"
                ></el-input>
              </el-form-item>
              ...
              <el-form-item label="名称">
                <el-input
                  placeholder="名称"
                  v-model="searchInfo.province_name"
                ></el-input>
              </el-form-item>
    ...
    >
      ...
      <el-table-column label="省份ID" prop="province_id" show-overflow-tooltip>
      </el-table-column>

      <el-table-column label="名称" prop="province_name" show-overflow-tooltip>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="名称" prop="province_name">
          <el-input
    ...
    <uploadexcel ref="uploadexcel" action="SysCommonProvinceInfo"></uploadexcel>
  </div>
</template>

<script>
import {
  createSysCommonProvinceInfo,
  deleteSysCommonProvinceInfo,
  updateSysCommonProvinceInfo,
  findSysCommonProvinceInfo,
  getSysCommonProvinceInfoList,
  batchSysCommonProvinceInfoOperation,
  getSysCommonProvinceInfoSummary,
} from "@/api/common/common_province_info";
...
export default {
  ...
  data() {
    return {
      listApi: getSysCommonProvinceInfoList,
  ...
  methods: {
    ...
    async editRow(row) {
      this.type = "update";
      this.dialogTitle = "编辑";
      const res = await findSysCommonProvinceInfo({ ID: row.ID });
      if (res.code == 0) {
        this.formData = res.data.recommon_province_info;
        this.dialogFormVisible = true;
      }
    },
    deleteRow(row) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        this.deleteSysCommonProvinceInfo(row);
      });
    },
    async deleteSysCommonProvinceInfo(row) {
      const res = await deleteSysCommonProvinceInfo({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功",
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createSysCommonProvinceInfo(this.formData);
          break;
        case "update":
          res = await updateSysCommonProvinceInfo(this.formData);
          break;
        default:
          this.$message({
            type: "error",
    ...
    handleCommand(command) {
      this.$confirm("是否要执行批量操作?", "提示", {
        ...
        const res = await batchSysCommonProvinceInfoOperation({
          ids,
          command: command,
        });
        if (res.code == 0) {
    ...
    async getSummaryList() {
      const res = await getSysCommonProvinceInfoSummary(this.searchInfo);
    ...
    async exportExcel() {
      this.searchInfo.action = "SysCommonProvinceInfo";
