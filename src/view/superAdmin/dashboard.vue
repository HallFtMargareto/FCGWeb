  ...
  <div class="dashboard-container">
    ...
    <el-card class="welcome-banner" shadow="never">
      <div class="banner-content">
        <div class="avatar-section">
          <el-avatar :size="60" :src="dashboardData.siteInfo.logo" class="user-avatar"></el-avatar>
        </div>
        <div class="info-section">
          <h2>{{ dashboardData.greeting }}，欢迎使用 {{ dashboardData.siteInfo.site_name }}</h2>
          <p class="subtitle">福彩数据分析平台</p>
        </div>
        <div class="date-section">
          <el-date-picker v-model="dashboardData.selectedDate" type="date" placeholder="选择日期" size="small"
            class="date-picker" />
        </div>
      </div>
    </el-card>
    ...
    <el-card class="section-card" shadow="never">
      <div slot="header" class="card-header">
        <span class="section-title">待处理投注</span>
      </div>
      <el-row :gutter="20">
        <el-col :span="4" v-for="item in dashboardData.pendingOrders" :key="item.key">
...
export default {
  ...
  data() {
    return {
      dashboardData: {
        ...
        // 待处理投注
        pendingOrders: [
          { key: "recharging", icon: "el-icon-position", value: 0, label: "待开奖", colorClass: "warning" },
          { key: "await", icon: "el-icon-loading", value: 0, label: "待支付", colorClass: "warning" },
          { key: "delay", icon: "el-icon-stopwatch", value: 0, label: "支付失败", colorClass: "warning" },
          { key: "hold", icon: "el-icon-video-pause", value: 0, label: "已撤单", colorClass: "warning" },
          { key: "failHold", icon: "el-icon-circle-close", value: 0, label: "退款中", colorClass: "warning" },
          { key: "unknown", icon: "el-icon-warning-outline", value: 0, label: "异常订单", colorClass: "warning" }
        ],
  ...
  async created() {
    ...
    // 更新站点信息
    if (this.siteInfo) {
      this.dashboardData.siteInfo = this.siteInfo;
    }
  }
};
</script>
