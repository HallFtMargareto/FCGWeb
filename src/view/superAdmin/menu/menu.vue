<template>
  <div>
    ...
    <el-table :data="tableData" border row-key="ID" stripe>
      <el-table-column label="名称" min-width="120" prop="authorityName">
        <template slot-scope="scope">
          <span><i :class="`el-icon-${scope.row.meta.icon}`"></i>{{ scope.row.meta.title }}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column
        label="路由Name"
      ...
      <el-table-column label="文件路径" min-width="360" prop="component" show-overflow-tooltip></el-table-column>
      <el-table-column fixed="right" label="操作" width="300">
        <template slot-scope="scope">
          <el-button @click="addMenu(scope.row.ID)" size="small" type="text" icon="el-icon-edit">添加子菜单</el-button>
          <el-button @click="editMenu(scope.row.ID)" size="small" type="text" icon="el-icon-edit">编辑</el-button>
          <el-button @click="deleteMenu(scope.row.ID)" size="small" type="text" icon="el-icon-delete">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form :inline="true" :model="form" :rules="rules" label-position="top" label-width="85px" ref="menuForm"
        ...
        <el-form-item prop="path" style="width: 30%">
          <div style="display: inline-block" slot="label">
            路由path<el-checkbox style="float: right; margin-left: 20px" v-model="checkFlag">添加参数</el-checkbox>
          </div>
          <el-input :disabled="!checkFlag" autocomplete="off" placeholder="建议只在后方拼接参数" v-model="form.path"></el-input>
        </el-form-item>
        ...
        <el-form-item label="父节点Id" style="width: 30%">
          <el-cascader :disabled="!this.isEdit" :options="menuOption" :props="{
            checkStrictly: true,
            label: 'title',
            value: 'ID',
            disabled: 'disabled',
        ...
        <el-form-item label="图标" prop="meta.icon" style="width: 30%">
          <icon :meta="form.meta" style="padding: 0">
...
export default {
  ...
  methods: {
    ...
    async editMenu(id) {
      ...
      const res = await getBaseMenuById({
        id,
      });
