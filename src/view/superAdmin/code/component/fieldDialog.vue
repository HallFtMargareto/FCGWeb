...
export default {
  ...
  methods: {
    autoFill() {
      this.dialogMiddle.fieldJson = this.$utils.toLowerCase(
        this.dialogMiddle.fieldName
      );
      this.dialogMiddle.columnName = this.$utils.toSQLLine(
        this.dialogMiddle.fieldJson
      );
    },
    async getDbfdOptions() {
      ...
      if (this.dialogMiddle.fieldType) {
        const res = await this.$utils.getDict(this.dialogMiddle.fieldType);
