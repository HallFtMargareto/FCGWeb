<template>
  <div>
    ...
    >
      ...
      <el-form-item label="API路径" prop="abbreviation">
        <el-input
...
import PreviewCodeDialg from "./component/previewCodeDialg.vue";
import {
  createTemp,
  getDB,
  getTable,
  ...
  methods: {
    ...
    enterDialog() {
      this.$refs.fieldDialog.$refs.fieldDialogFrom.validate((valid) => {
        if (valid) {
          this.dialogMiddle.fieldName = this.$utils.toUpperCase(
    ...
    async enterForm(isPreview) {
      ...
      this.$refs.autoCodeForm.validate(async (valid) => {
        if (valid) {
          this.form.structName = this.$utils.toUpperCase(this.form.structName);
    ...
    async getColumn() {
      ...
      if (res.code == 0) {
        const tbHump = this.$utils.toHump(this.dbform.tableName);
        this.form.structName = this.$utils.toUpperCase(tbHump);
        ...
        res.data.columns &&
          res.data.columns.map((item) => {
            if (!gormModelList.some((gormfd) => gormfd == item.columnName)) {
              const fbHump = this.$utils.toHump(item.columnName);
              this.form.fields.push({
                fieldName: this.$utils.toUpperCase(fbHump),
                fieldDesc: item.columnComment || fbHump,
                fieldType: this.fdMap[item.dataType],
                dataType: item.dataType,
                // fieldJson: fbHump,
                fieldJson: item.columnName,
    ...
    async setFdMap() {
      ...
      fdTypes.map(async (fdtype) => {
        const res = await this.$utils.getDict(fdtype);
        res &&
          res.map((item) => {
