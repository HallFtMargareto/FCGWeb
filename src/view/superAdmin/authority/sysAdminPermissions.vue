<template>
  <div>
    ...
    >
      ...
      <el-table-column label="日期" prop="created_at" sortable="custom">
    ...
    <uploadexcel ref="uploadexcel" action="SysAdminPermissions"></uploadexcel>
  </div>
</template>

<script>
import {
  createSysAdminPermission,
  deleteSysAdminPermission,
  updateSysAdminPermission,
  findSysAdminPermission,
  getSysAdminPermissionList,
  batchSysAdminPermissionOperation,
  getSysAdminPermissionSummary,
...
export default {
  ...
  data() {
    return {
      listApi: getSysAdminPermissionList,
      ...
      formDataRules: {
        describe: [{ required: true, message: "请填写数据", trigger: "blur" }],
        resource: [{ required: true, message: "请填写数据", trigger: "blur" }],
        action: [{ required: true, message: "请填写数据", trigger: "blur" }],
      },
      miniDataList: [],
    };
  },
  methods: {
    ...
    async editRow(row) {
      this.type = "update";
      this.dialogTitle = "编辑";
      const res = await findSysAdminPermission({ ID: row.ID });
      if (res.code == 0) {
        this.formData = res.data.resysAdminPermissions;
        this.dialogFormVisible = true;
      }
    },
    deleteRow(row) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        this.deleteSysAdminPermissions(row);
      });
    },
    async deleteSysAdminPermissions(row) {
      const res = await deleteSysAdminPermission({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功",
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createSysAdminPermission(this.formData);
          break;
        case "update":
          res = await updateSysAdminPermission(this.formData);
          break;
        default:
          this.$message({
            type: "error",
    ...
    async getSummaryList() {
      const res = await getSysAdminPermissionSummary(this.searchInfo);
    ...
    async exportExcel() {
      this.searchInfo.action = "SysAdminPermissions";
