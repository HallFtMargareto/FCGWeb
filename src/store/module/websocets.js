...
export const websocets = {
    ...
    mutations: {
        //初始化ws 用户登录后调用
        initSocket(state, baseInfo) {

            if (state.baseInfo == null) {
            ...
            state.ws.onclose = function (res) {
                ...
                //重连
                that.commit('websocets/reconnect');
            }
        ...
        reconnect(state) {
            ...
            state.timeoutnum = setTimeout(() => {
                //新连接
                that.commit('websocets/initSocket')
        ...
        start(state) {
            ...
            state.timeoutObj = setTimeout(() => {
                ...
                if (state.ws.readyState === 1) {
                    ...
                    //否则重连
                    self.commit('websocets/reconnect');
                }
    ...
    actions: {
        async webSocketInit(content) {
            ...
            content.commit('initSocket', res.data)
        },
        send({ commit }, data) {
