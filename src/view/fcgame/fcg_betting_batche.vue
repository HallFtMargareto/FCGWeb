...
export default {
  ...
  data() {
    return {
      listApi: getFcgBettingBatcheList,
      openDialog: false,
      dialogTitle: "",
      type: "",
      multipleSelection: [],
  ...
  methods: {
    ...
    async editRow(row) {
      this.type = "update";
      this.dialogTitle = "编辑";
      const res = await findFcgBettingBatche({ ID: row.ID });
      if (res.code == 0) {
        this.formData = res.data.refcg_betting_batche;
        this.openDialog = true;
      }
    },
    async deleteRow(row) {
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createFcgBettingBatche(this.formData);
          break;
        case "update":
          res = await updateFcgBettingBatche(this.formData);
          break;
        default:
          this.$message({
            type: "error",
    ...
    async getSummaryList() {
      const res = await getFcgBettingBatcheSummary(this.searchInfo);
    ...
    async exportExcel() {
      this.searchInfo.action = "fcg_betting_batche";
